...
async function ensureCharacterCatalog(db, work) {
  ...
  for (const char of result.characters) {
    await db.updateOne(
      { _id: `character:${char.source}:${char.charId}` },
      {
        $set: {
          name: char.name,
          image: char.image,
...
async function hasRolledToday(db, todayKey) {
  const state = await db.findOne({ _id: "character-event:state" });
  return state?.lastRolledDate === todayKey;
}

async function markRolledToday(db, todayKey) {
  await db.updateOne(
    { _id: "character-event:state" },
    { $set: { kind: "characterEventState", lastRolledDate: todayKey } },
    { upsert: true }
  );
}
...
async function rollDailyEvent(db) {
  ...
  if (await hasRolledToday(db, todayKey)) return null;
  await markRolledToday(db, todayKey); // se marca ya, salga o no personaje, para no reintentar hoy
